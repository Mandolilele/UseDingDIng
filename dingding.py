...
# 是否在指定时间后随机时间打卡，以免每次都打卡都是明确的指定时间，比如8:30 
USEDELAYTIME = True
# USEDELAYTIME = False
...
COORDINATES = [(433, 2268),(150,980), (560, 1400)] # for xiaomi 12 pro & 红米note 7 pro
...
# 设备连接信息
DEVICE_IP = "xxx.xxx.xxx.xxx"   # 设备在局域网的地址
DEVICE_PORT = "5555"
device_ip = f"{DEVICE_IP}:{DEVICE_PORT}"

# 邮件配置
SMTP_SERVER = 'smtp.qq.com'                 #发送通知邮件的服务器信息
SMTP_PORT = 587
SMTP_USERNAME = '888888@qq.com'
SMTP_PASSWORD = '***************'
EMAIL_FROM = '6666666@qq.com'

EMAIL_TO = 'name@mail.com'
EMAIL_SUBJECT = 'DingDing Screenshot Result'

# 日志记录函数
def log(message):
    with open("/Users/mandoli/code/09DD/dingdingpylog.log", "a") as log_file:       # 这个自己设定
...
def launch_dingding(device_id=None):
    ...
    time.sleep(10)  # 等待应用启动  这个间隔时间要根据手机的反应速度来设定，有些打卡的机器很慢，所以间隔就要长一点
